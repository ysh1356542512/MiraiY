...
suspend fun main() {
    ...
    val bot = BotFactory.newBot(ControlCenter.botQQ, "xxxxxxx") {
...
//                    delay(1000 * 10)
        ...
        launch {
            while (true) {
                if (isBelong("12:00", "23:59") && !ControlCenter.isTaoFinished) {
//                if (isBelong("19:46", "20:30") && !ControlCenter.isOneFinished) {
                    println("开始检测")
                    //开始检测
...
//                    delay(1000 * 10)
        ...
        launch {
            while (true) {
                if (isBelong("00:00", "01:00")) {
                    ...
                    ControlCenter.isCardTodayFinished = false
                    ControlCenter.clockTime = (Math.random() * 50 + 10).toInt()
                    ...
                    for (stu in ControlCenter.studentTao) {
                        stu.isTodayChecked = false
                    }
                    delay(1000 * 60 * 45)
                } else {
        ...
        launch {
            while (true) {
                if (isBelong("10:${ControlCenter.clockTime}", "12:00")&&!ControlCenter.isCardTodayFinished) {
                    DealMessage.clockGroupCard(ControlCenter.studentsCard)
                }
            }
        }
        //重置学生信息
